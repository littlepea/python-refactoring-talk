...
class TwitterSearchOrderTest(unittest.TestCase):
    ...
    def test_TSO_keywords(self):
        """ Tests TwitterSearchOrder.setKeywords() and .add_keyword() """
    ...
    def test_TSO_set_keywords_OR(self):
        """" Tests TwitterSearchOrder.setKeywords(or_operator) """

        tso = self.getCopy()
        keywords = ("bob","alice")
        tso.setKeywords(keywords, or_operator=True)
        self.assertTrue("+or+".join(keywords) in tso.create_search_url().lower())
